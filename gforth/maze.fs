...
\ tmp vars
variable HEIGHT variable WIDTH variable argslist variable tmph variable tmpw
: parse-args        
	argslist @ 0 = if
		cr tmpw @ tmph @
	else
		cr parse-width parse-height 
	then
;
: checkargs
	argc @ 2 < if
		cr cr ." must give WIDTH & HEIGHT as argument" cr
		." gforth thiscode.fs 20 10" cr cr cr
		25 tmph !  25 tmpw !  0 argslist !
	else
		1 argslist !
	then
;
...
: update-state          ( x y m d -- x' y' m' )     { x y m d }
                                                    x y d dir-valid? if
                                                        ...
                                                        2dup visited? if
                                                            ...
                                                            2dup visited!  x y d remove-wall 0
                                                        then
                                                    else
                                                        x y m
                                                    then ;   

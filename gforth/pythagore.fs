...
: exitcode ( -- ) 		\ quitter
	ESC emit s\" [0m" type 	\ restorer le terminal
	cr cr 
	0 (bye)	\ rendre la main au terminal 
;  
...
: #shortsegment ( a h -- b )	\ le petit segment
	...
	pad dup 20 accept >float fdup f*
	f- fabs					\ h² - a²
	fsqrt 5 set-precision	\ juste pour afficher proprement
	CR ." 	segment court : " f. 
	cr cr
;
...
: #longsegment ( b h -- a )	\ le long segment
	...
	cr ." valeur du second (hypotenuse) : "	\ H 
	pad dup 20 accept >float fdup f*
	f- fabs	fsqrt	5 set-precision
	CR ." 	segment long : " f.  cr cr
;
...
: 33colormenu ( -- ) \ juste un menu 
	cr ." Quelle est le segment manquant :" 
	cr 33 colorize
	." H 	-	hypotenuse" cr
	." S 	-	segment court" cr
	." L 	-	segment long" cr
	." Q 	-	quitter de suite" cr cr
; 
